...
(function($) {
  ...
  $.uiTableFilter = function(jq, phrase, column, ifHidden){
    var new_hidden = false,
        ...
        matches = function(elem) { elem.show(); },
    ...
    if( (words.size > 1) && (phrase.substr(0, phrase_length - 1) === this.last_phrase) ) {
      ...
      matches = function(elem) { elem = null; };
    ...
    elems.each(function(){
      var elem = $(this);
      if($.uiTableFilter.has_words( getText(elem), words, false )) { matches(elem); } else { noMatch(elem); }
    });
  ...
  $.uiTableFilter.has_words = function( str, words, caseSensitive ) {
    var text = (caseSensitive) ? str : str.toLowerCase(), i = "";
