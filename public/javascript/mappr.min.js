...
$("#mapOutputImage").attr("width",0).attr("height",0);$("#mapOutputScale").hide();$(this).parent().removeClass("mapTools-collapsed");$("#mapTools").show("slide",{direction:"right"},250,function(){$("#actionsBar").animate({width:"810px"},250);$("#map").animate({width:"800px"},function(){$('input[name="width"]').val(800);a.mapRefresh()})})})};Mappr.showMessage=function(a){$("#mapper-message").html(a).dialog({autoOpen:!0,height:(200).toString(),width:(400).toString(),modal:!0,closeOnEscape:!1,draggable:!0,
resizable:!1,buttons:[{text:"OK","class":"positive",click:function(){$(this).dialog("destroy")}}]}).show()};Mappr.drawLegend=function(){var a=$("#legend_url").val();a?$("#mapLegend").html('<img src="'+a+'" />'):$("#mapLegend").html("<p><em>"+$("#mapper-legend-message").text()+"</em></p>")};Mappr.drawScalebar=function(){$("#mapScale img").attr("src",$("#scalebar_url").val()).show()};Mappr.showBadPoints=function(){var a=$("#bad_points").val();a&&($("#badRecords").html(a),$("#badRecordsWarning").show())};
Mappr.showSavingMessage=function(a){a='<span class="mapper-loading-message ui-corner-all ui-widget-content">'+a+"</span>";this.hideErrorMessage();$("#mapOutput").append(a)};Mappr.hideSavingMessage=function(){$("#mapOutput .mapper-loading-message").remove()};Mappr.showSpinner=function(){$(".mapper-loading-spinner").show()};Mappr.hideSpinner=function(){$(".mapper-loading-spinner").hide()};Mappr.showErrorMessage=function(a){$("#mapOutput").append('<span class="mapper-message-error ui-corner-all ui-widget-content">'+
a+"</span>")};Mappr.hideErrorMessage=function(){$("#mapOutput .mapper-message-error").remove()};Mappr.showMap=function(a){var b=(new Date).getTime(),c="",d={};this.destroyJcrop();$("#output").val("pnga");$("#badRecordsWarning").hide();$("#download_token").val(b);c=$("form").serialize();d=$("form").serializeJSON();this.postData(c,a);$.jStorage.set("do-"+b.toString(),d);this.toggleUndo(!0)};Mappr.postData=function(a,b){var c=this;c.showSpinner();$.ajax({type:"POST",url:c.settings.baseUrl+"/application/",
data:a,dataType:"json",timeout:2E4,success:function(a){c.resetFormValues(a);c.resetJbbox();c.drawMap(a,b);c.drawLegend();c.drawScalebar();c.showBadPoints();c.addBadRecordsViewer()},error:function(a,b){"timeout"===b&&c.showErrorMessage($("#mapper-saving-error-message").text())}})};Mappr.resetFormValues=function(a){$("#mapOutput input").each(function(){$(this).val("")});$.each("rendered_bbox rendered_rotation rendered_projection legend_url scalebar_url bad_points".split(" "),function(){$("#"+this).val(a[this])});
$("#bbox_map").val($("#rendered_bbox").val());$("#projection_map").val($("#rendered_projection").val());$("#rotation").val($("#rendered_rotation").val());$("#pan").val("")};Mappr.drawMap=function(a,b){var c=this;$("#mapOutputImage").attr("width",a.size[0]).attr("height",a.size[1]).attr("src",a.mapOutputImage).one("load",function(){b||(b={map:{bbox_rubberband:""}});c.loadCropSettings(b);c.hideSpinner()})};Mappr.addBadRecordsViewer=function(){var a=this;$("#badRecordsViewer").dialog({autoOpen:!1,height:(200).toString(),
